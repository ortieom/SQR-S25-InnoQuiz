...
def add_question(
    ...
    quiz = repo.quiz.get_by_id(db, uuid.UUID(quiz_id))
    if quiz is None:
    ...
    options = repo.answer_option.get_by_question_id(db, question_obj.id)
...
def load_external_questions(
    ...
    for q in trivia_questions:
        ...
        question_options = repo.answer_option.get_by_question_id(db, question_obj.id)
...
def get_quiz_info(quiz_id: str, db: Session) -> QuizInfoResponse:
    ...
    question_count = repo.question.count_by_quiz_id(db, uuid.UUID(quiz_id))
...
def get_leaderboard(quiz_id: str, db: Session) -> LeaderboardResponse:
    ...
    attempts = repo.user_attempt.get_by_quiz_id(db, uuid.UUID(quiz_id))
...
def get_quiz_questions(quiz_id: str, db: Session) -> QuizQuestionsResponse:
    ...
    questions = repo.question.get_by_quiz_id(db, uuid.UUID(quiz_id))

    question_responses = []
    for q in questions:
        options = repo.answer_option.get_by_question_id(db, q.id)
...
def submit_quiz_answers(
    ...
    """
    quiz = repo.quiz.get_by_id(db, uuid.UUID(request.quiz_id))
    if quiz is None:
        raise errors.QuizNotFoundError()
    ...
    # Calculate score
    total_questions = repo.question.count_by_quiz_id(db, uuid.UUID(request.quiz_id))
    correct_count = 0

    for answer in request.answers:
        # Try to convert question_id to UUID, but handle case where it might be a different format
        try:
            question_uuid = uuid.UUID(answer.question_id)
        ...
        options = repo.answer_option.get_by_question_id(db, question.id)
    ...
    rank = None
    attempts = repo.user_attempt.get_by_quiz_id(db, uuid.UUID(request.quiz_id))
    if attempts:
